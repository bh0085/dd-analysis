...
def process_dataset(gcs_folder, dataset, dataset_key):
    ...
    reset_jobs = ["INIT_TOPHAT"]
    for j in reset_jobs:
        set_jobstatus(dataset_key, jobs[j], status["WAITING"])
        set_jobprogress(dataset_key,jobs[j], 0)
            
    #process input xumi data to coords / annotations formats for the frontend
    if get_jobstatus(dataset_key,jobs["INIT_FRONTEND"]) == status["WAITING"]:
    ...
    if get_jobstatus(dataset_key,jobs["INIT_BLAT"]) == status["WAITING"]:
    ...
    if get_jobstatus(dataset_key, jobs["INIT_TOPHAT"]) == "WAITING":
        ...
        output_status = init_tophat(tmpdir,dataset)
                              
        if output_status == 0:
            set_jobstatus(dataset_key, jobs["INIT_TOPHAT"], status["COMPLETE"])
        if output_status == -1:
            set_jobprogress(dataset_key, jobs["INIT_TOPHAT"], 25)
        else: raise Exception(f"unhandled nonzero output status for {jobs['INIT_TOPHAT']}")
...
def get_jobprogress(dskey, name):
    val = root.get()[dskey]
    status = val["server_job_progresses"][name]
    return status    
    
def set_jobprogress(dskey, name, progress):
    val = root.get()[dskey]
    val["server_job_progresses"][name] = progress
    root.update({dskey:val})
